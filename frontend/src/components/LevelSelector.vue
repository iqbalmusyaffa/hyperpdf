      ...
      >
        ...
        <div class="flex items-center justify-between mb-3">
          <div class="flex items-center space-x-1.5 flex-wrap gap-1">
            ...
            <span
              v-if="opt.isProOnly"
              class="px-2 py-0.5 text-[10px] font-extrabold rounded-full bg-gradient-to-r from-amber-500 to-rose-500 text-white shadow-xs"
            >
              PRO ⭐
            </span>
            <span
...
const { isPro, remainingQuota, maxDailyLimit } = useAuth()

function handleSelect(opt: LevelOption) {
  if (opt.isProOnly && !isPro.value) {
    emit('selectLevel', opt.id)
    emit('requirePro')
    return
  }
  emit('selectLevel', opt.id)
...
function handleCompressClick() {
  const currentOpt = props.options.find((o) => o.id === props.selectedLevel)
  if (currentOpt?.isProOnly && !isPro.value) {
    emit('requirePro')
    return
  }
  emit('compress')
