<template>
  <div class="bg-white border border-slate-200 rounded-3xl p-8 sm:p-12 shadow-sm text-center max-w-2xl mx-auto space-y-8">
    <!-- Success Badge / Icon -->
    ...
    <div class="space-y-1">
      ...
      <p class="text-sm sm:text-base text-slate-500">
        File compressed with <span class="font-bold text-slate-700">{{ result.compression_level }}</span> optimization.
      </p>
    </div>
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-2xl p-6 space-y-5">
      <div class="grid grid-cols-3 gap-4 text-center">
        <!-- Original Size -->
        <div class="space-y-1">
          <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Original</p>
          <p class="text-lg sm:text-xl font-bold text-slate-700">{{ originalSizeFormatted }}</p>
        </div>

        <!-- Compressed Size -->
        <div class="space-y-1">
          <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Compressed</p>
          <p class="text-lg sm:text-xl font-extrabold text-emerald-600">{{ compressedSizeFormatted }}</p>
        </div>

        <!-- Saved Percentage -->
        <div class="space-y-1">
          <p class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Saved</p>
          <p class="text-lg sm:text-xl font-extrabold text-brand-600">-{{ result.compression_percentage }}%</p>
        </div>
      </div>

      <!-- Reduction bar -->
      <div class="space-y-1.5 pt-2 border-t border-slate-200/60">
        <div class="w-full bg-slate-200 rounded-full h-3 overflow-hidden p-0.5">
          <div
        ...
        <p class="text-xs text-slate-500 font-medium">
          You saved <span class="font-bold text-slate-800">{{ savedBytesFormatted }}</span> of storage space.
        </p>
      </div>
    </div>
