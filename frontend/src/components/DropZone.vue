<template>
  <div class="w-full">
    ...
    >
      ...
      <div class="flex flex-col items-center justify-center space-y-4">
        <div
          ...
          :class="isDragging ? 'bg-brand-500 text-white' : 'bg-red-50 dark:bg-red-900/30 text-brand-500'"
        >
        ...
        <div class="space-y-1.5">
          <p class="text-xl font-bold text-slate-800 dark:text-white">
            <span class="text-brand-600 dark:text-brand-400 group-hover:underline">Pilih file PDF</span> atau tarik & lepas di sini
          </p>
          <p class="text-sm text-slate-500 dark:text-slate-400">
            Hingga 50 MB per file • Dilengkapi live page preview & HyperPDF engine
          </p>
        </div>
    ...
    <!-- File Selected View with PDF Thumbnail Preview -->
    <div
      v-else
      class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6"
    >
      <div class="flex items-center space-x-5 w-full sm:w-auto">
        <!-- Thumbnail Preview or Fallback Icon -->
        <div class="relative w-20 h-24 rounded-2xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 overflow-hidden shrink-0 shadow-sm flex items-center justify-center">
          <img
            v-if="thumbnailUrl"
            :src="thumbnailUrl"
            alt="PDF Preview"
          ...
          />
          <div v-else class="text-brand-500 flex flex-col items-center justify-center p-2 text-center">
            ...
            <span class="text-[9px] font-bold mt-1 text-slate-400">PDF</span>
          </div>

          <!-- Page count badge -->
          <span
            v-if="pageCount > 0"
            class="absolute bottom-1 right-1 bg-slate-900/80 text-white text-[9px] font-bold px-1.5 py-0.2 rounded-md backdrop-blur-xs"
          >
            {{ pageCount }} hal
          </span>
        </div>
        ...
        <div class="truncate">
          <div class="flex items-center space-x-2">
            <h4 class="text-base sm:text-lg font-bold text-slate-800 dark:text-white truncate max-w-xs sm:max-w-md">
              {{ file.name }}
            </h4>
          ...
          <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium mt-1">
            Ukuran Asli: <span class="font-semibold text-slate-700 dark:text-slate-300">{{ formattedSize }}</span>
            <span v-if="pageCount > 0"> • {{ pageCount }} Halaman</span>
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center space-x-3 w-full sm:w-auto justify-end">
        <button
          ...
          type="button"
          @click="$emit('clear')"
          class="px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-600 dark:text-slate-300 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40 border border-slate-200 dark:border-slate-700 transition-colors flex items-center space-x-1.5"
        >
...
watch(
  ...
  async (newFile) => {
    if (newFile) {
      const res = await generateThumbnail(newFile, 0.4)
